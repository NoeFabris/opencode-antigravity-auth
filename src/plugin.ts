        ...
        async fetch(input, init) {
          ...
          while (true) {
            ...
            const account = accountManager.getCurrentOrNextForFamily(
              ...
              preferredHeaderStyle,  // Respects cli_first config for quota selection
            ...
            // Use preferredHeaderStyle computed above (respects cli_first config)
            let headerStyle = preferredHeaderStyle;
            pushDebug(`headerStyle=${headerStyle} explicit=${explicitQuota} cliFirst=${cliFirst}`);
...
function getHeaderStyleFromUrl(urlString: string, family: ModelFamily, cliFirst: boolean = false): HeaderStyle {
  if (family === "claude") {
    return "antigravity";
  }
