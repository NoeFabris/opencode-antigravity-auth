                ...
                for (let i = 0; i < ANTIGRAVITY_ENDPOINT_FALLBACKS.length; i++) {
                  ...
                  try {
                    ...
                    if (!response.ok) {
                      ...
                      // fresh accounts sometimes get 403s bc google hasnt provisioned them yet
                      // just show a friendly msg so ppl know to wait a bit
                      if (response.status === 403) {
                        const cloned = response.clone();
                        const bodyText = await cloned.text();
                        if (isCloudAICompanionPermissionDenied(bodyText)) {
                          if (!quietMode) {
                            await showToast(
                              getProvisioningErrorMessage(),
                              "warning"
